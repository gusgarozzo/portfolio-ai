import type { Certification } from "@/types/portfolio";

const es: Certification[] = [
  {
    name: "Arquitectura de Software con IA: Aplicación Práctica",
    issuer: "LinkedIn",
    year: 2026,
    featured: true,
  },
  {
    name: "Programación en Pareja (Pair Programming) con IA",
    issuer: "LinkedIn",
    year: 2026,
  },
  { name: "Depuración de Código: Node", issuer: "LinkedIn", year: 2026, featured: true },
  {
    name: "Arquitectura de Software: Diseño Guiado por el Dominio (DDD)",
    issuer: "LinkedIn",
    year: 2026,
    featured: true,
  },
  { name: "Domina la IA con Gemini", issuer: "Google", year: 2025 },
  {
    name: "Introducción a la IA Generativa",
    issuer: "MIT Professional Education",
    year: 2025,
  },
  {
    name: "Lidera a Máxima Velocidad: Lecciones de la Fórmula 1",
    issuer: "Santander",
    year: 2025,
  },
  {
    name: "Introducción a AWS: Conceptos de la Nube",
    issuer: "LinkedIn",
    year: 2025,
    featured: true,
  },
  {
    name: "Ingeniería de Prompts: Aprende a Hablar con una IA Generativa",
    issuer: "LinkedIn",
    year: 2025,
  },
  {
    name: "Arquitectura de Software: Patrones Esenciales",
    issuer: "LinkedIn",
    year: 2023,
  },
];

const en: Certification[] = [
  {
    name: "Software Architecture with AI: Practical Application",
    issuer: "LinkedIn",
    year: 2026,
    featured: true,
  },
  {
    name: "Pair Programming with AI",
    issuer: "LinkedIn",
    year: 2026,
  },
  { name: "Code Debugging: Node", issuer: "LinkedIn", year: 2026, featured: true },
  {
    name: "Software Architecture: Domain-Driven Design (DDD)",
    issuer: "LinkedIn",
    year: 2026,
    featured: true,
  },
  { name: "Mastering AI with Gemini", issuer: "Google", year: 2025 },
  {
    name: "Introduction to Generative AI",
    issuer: "MIT Professional Education",
    year: 2025,
  },
  {
    name: "Lead at Full Speed: Lessons from Formula 1",
    issuer: "Santander",
    year: 2025,
  },
  {
    name: "Introduction to AWS: Cloud Concepts",
    issuer: "LinkedIn",
    year: 2025,
    featured: true,
  },
  {
    name: "Prompt Engineering: How to Talk to Generative AI",
    issuer: "LinkedIn",
    year: 2025,
  },
  {
    name: "Software Architecture: Essential Patterns",
    issuer: "LinkedIn",
    year: 2023,
  },
];

export const certifications = { es, en };
